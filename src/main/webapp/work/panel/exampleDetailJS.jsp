...
//        System.out.println("Target DIR: "+fskt.getPath());
        ...
        for(File f:flist){
            ...
            if(f.getName().startsWith(skt)&&f.getName().equals(skt+".js")&&!skt.equals(skt_n_name)){
                tof = new File(toDir+"/"+skt_n_name+".js");    
            } else {
        ...
        response.sendRedirect("sketcherDetailJS?k="+params.setDataValues("fn",skt+".js","skt",skt,"act","")+"&_rm=true");
        return;
    }
...
<section class="content">
    <div class="row">
        ...
        if (code != null) {
                ...
                %>
                <input type="button" value="Send" onclick="document.getElementById('consoleLog').value = 'Compiling...\n\r';getAsynchData('exampleDetailJS?k=<%=params.setDataValues("cp",(filename != null ? filename : ""),"skt",skt,"fn",filename)%>&dev=' + document.getElementById('type').value, myCodeMirror.getValue(), 'POST',function(data){document.getElementById('consoleLog').value = data;});" class="btn btn-primary" >
                <a class="btn btn-primary" data-target=".content-wrapper" data-load="ajax" href="exampleDetailJS?k=<%=params.setDataValues("clone",(filename != null ? filename : ""),"skt",skt,"fn",filename)%>">Clone to Sketchers</a>
                <% 
                    }
                %>
                    </div>
